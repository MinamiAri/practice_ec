window.onload = function () {
  ...
  // カートボタンを押した際の処理
  cart_btns.forEach(function (cart_btn,index) {//カートボタンを押して
  ...
  //追加部分
  const selected = document.getElementById("js-sort");

  itemList.forEach(item => {
    selected.onchange = function() {
      let type =item.dataset.type;
      let sortValue = selected.value;
      
      const targetElements = document.getElementsByClassName('sort-out');
      if(sortValue === type) {
        let itemClass = document.querySelectorAll(sortValue);
        for(let i =0; i < itemClass.length; i++) {
          itemClass.classList.add('sort-out');
        }
      } else if (sortValue === 'all') {
        targetElements.forEach.call(targetElements, function(e) {
          e.classList.remove('sort-out');
        })
      }
